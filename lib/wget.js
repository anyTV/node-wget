    ...
    req = request(
        ...
        function(res) {
            ...
            if (res.statusCode === 200 || res.statusCode === 206) {
                ...
                if (!isNaN(options.start) && options.total_size) {
                    // need to create buffer file whose size === total_size for createWriteStream's `start` option
                    // reference: https://stackoverflow.com/a/16106581
                    fs.writeFileSync(output, Buffer.alloc(options.total_size), { flags: 'wx' });
                    writeStreamOptions.start = options.start;
                    writeStreamOptions.flags = 'r+';
                }

                writeStream = fs.createWriteStream(output, writeStreamOptions);
